...
pop.3
hist(DataDB$Population)
plot(DataDB$Population)
...
pop50.mex <-  DataDB %>% 
  filter(CountryCode == "MEX" ,  
         Population > 50000)   # Ciudades del país de México con más de 50,000 habitantes
...
# Nos descontactams de la BDD
dbDisconnect(MyDataBase)

setwd("D:/0PERSONAL KITZYA/BEDU DATA ANALYSIS/SESION 7 - 02 JUNIO")
write.csv(DataDB, "DatadB.csv")
