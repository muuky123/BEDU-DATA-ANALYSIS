...
(var.hola <- "Hola Mundo") #Variable tipo caracter
(var.number <- 5L) #Variable númerica tipo entera
(var.double <- 2.7076)  #variable númerica double o decimal o flotantes (float) o racional
(var.logical <- T) #Variable tipo lógico T,F
(vector <- c(1,2)) #Guardas datos
...
(vector.by2 <- seq(from = 1, to = 10, by =2)) #numeros de 2 en 2 del 1 al 10


(vector.by3 <- seq(1, 10, 3)) #se pueden omitir el from, to, by y funciona igual
...
c(1, 2) + c(7, 8, 9, 10)  # Sumo el 1 y el 2 en los 2 primeros números y repitió la operación en los siguientes 2 números
